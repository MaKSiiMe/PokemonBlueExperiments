...
def main():
    ...
    base = "/home/maxime/Holberton/C#25-ML_Portfolio/"
    base += "PokemonBlueExperiments/data/dataset/pokemon"
    val_images = os.path.join(base, "val", "images")
    if not os.path.isdir(val_images):
        print(f"⚠️ Aucun dossier de validation trouvé à: {val_images}.")
        print("Utilisation de train/images pour la validation.")
    ...
    for key in ("train", "val"):
        ...
        if not p or not os.path.isdir(p):
            print(f"❌ Chemin '{key}' introuvable: {p}.")
